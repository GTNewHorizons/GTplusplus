...
public class GregtechMetaTileEntity_IndustrialMacerator extends GregtechMeta_MultiBlockBase<GregtechMetaTileEntity_IndustrialMacerator> {
	...
	protected GT_Multiblock_Tooltip_Builder createTooltip() {
		...
		.addInfo("Increased output chances on % outputs")
	...
	public IStructureDefinition<GregtechMetaTileEntity_IndustrialMacerator> getStructureDefinition() {
		if (this.STRUCTURE_DEFINITION == null) {
			this.STRUCTURE_DEFINITION = StructureDefinition.<GregtechMetaTileEntity_IndustrialMacerator>builder()
	...
	public void construct(ItemStack stackSize, boolean hintsOnly) {
		...
		buildPiece(this.mName + "mid", stackSize, hintsOnly, 1, 1, 0);
		buildPiece(this.mName + "mid", stackSize, hintsOnly, 1, 2, 0);
		buildPiece(this.mName + "mid", stackSize, hintsOnly, 1, 3, 0);
		buildPiece(this.mName + "mid", stackSize, hintsOnly, 1, 4, 0);
		buildPiece(this.mName + "top", stackSize, hintsOnly, 1, 5, 0);
	}
	...
	public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
		...
		if (checkPiece(this.mName + "bottom", 1, 0 ,0)) {
			while (this.mPerLayer < 4) {
				if (!checkPiece(this.mName + "mid", 1, this.mPerLayer + 1, 0) ||
						this.mPerLayer + 1 != this.mOutputBusses.size()) return false;
				this.mPerLayer ++;
			}
			return checkPiece(this.mName + "top", 1, 5, 0) && this.mOutputBusses.size() == 5 && this.mCasing >= 26 && checkHatch();
		}
		return false;
	}
