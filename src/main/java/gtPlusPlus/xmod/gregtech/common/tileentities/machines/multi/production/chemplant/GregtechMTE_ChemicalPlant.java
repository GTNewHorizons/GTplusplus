...
public class GregtechMTE_ChemicalPlant extends GregtechMeta_MultiBlockBase<GregtechMTE_ChemicalPlant> {
	...
	public void construct(ItemStack stackSize, boolean hintsOnly) {
		buildPiece(this.mName , stackSize, hintsOnly, 3, 6, 0);
	}
	...
	public void updateHatchTexture() {
		for (GT_MetaTileEntity_Hatch h : this.mCatalystBuses) h.updateTexture(getCasingTextureID());
		for (GT_MetaTileEntity_Hatch h : this.mInputBusses) h.updateTexture(getCasingTextureID());
		for (GT_MetaTileEntity_Hatch h : this.mMaintenanceHatches) h.updateTexture(getCasingTextureID());
		for (GT_MetaTileEntity_Hatch h : this.mEnergyHatches) h.updateTexture(getCasingTextureID());
		for (GT_MetaTileEntity_Hatch h : this.mOutputBusses) h.updateTexture(getCasingTextureID());
		for (GT_MetaTileEntity_Hatch h : this.mInputHatches) h.updateTexture(getCasingTextureID());
		for (GT_MetaTileEntity_Hatch h : this.mOutputHatches) h.updateTexture(getCasingTextureID());
	}
	...
	public void loadNBTData(NBTTagCompound aNBT) {
		...
		this.mSolidCasingTier = aNBT.getInteger("mSolidCasingTier");
		this.mMachineCasingTier = aNBT.getInteger("mMachineCasingTier");
		this.mPipeCasingTier = aNBT.getInteger("mPipeCasingTier");
		this.mCoilTier = aNBT.getInteger("mCoilTier");
	}
	...
	public boolean addToMachineList(IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
		...
		if (aMetaTileEntity instanceof GT_MetaTileEntity_Hatch_Catalysts) {
			...
			return addToMachineListInternal(this.mCatalystBuses, aMetaTileEntity, aBaseCasingIndex);
		}
	...
	public boolean checkRecipeGeneric(
		...
		GT_Recipe tRecipe = findRecipe(this.mLastRecipe, gregtech.api.enums.GT_Values.V[tTier], getSolidCasingTier(), aItemInputs, aFluidInputs);
	...
	private static ItemStack findCatalyst(ItemStack[] aItemInputs, ItemStack[] aRecipeInputs) {
		if (aItemInputs != null) {
			for (final ItemStack aInput : aItemInputs) {
				if (aInput != null) {
	...
	private static void setDamage(ItemStack aStack,int aAmount) {
